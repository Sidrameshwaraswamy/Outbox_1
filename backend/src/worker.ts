...
export const worker = new Worker<DeliveryJob>(
  'email-delivery',
  async (job) => {
    ...
    if (!slot.allowed) {
      ...
      await emailQueue.add('deliver-email', job.data, {
        jobId: `${email.id}:${slot.nextHour.toISOString()}`,
        delay: slot.nextHour.getTime() - Date.now(),
        attempts: 5,
        backoff: {
          type: 'exponential',
          delay: 5000,
    ...
    const wait = Math.max(
      0,
      config.minSendDelayMs - (Date.now() - lastSendAt)
    );
